...
function onEdit(e) {
  ...
  // Headers: timestamp(A), userId(B), name(C), birth(D), theme(E), gptDraft(F), status(G), editedText(H), sentAt(I)
  if (s.getName() !== TARGET_SHEET_NAME || editedColumn !== 7 || e.value !== '検閲済') { 
    Logger.log('Edit ignored: Sheet=' + s.getName() + ', Col=' + editedColumn + ', Value=' + e.value + ', Expected Sheet: ' + TARGET_SHEET_NAME + ', Expected Col: 7, Expected Value: 検閲済');
    return;
  }
  ...
  // Indices: 0=A, 1=B, 2=C, 3=D, 4=E, 5=F(gptDraft), 6=G(status), 7=H(editedText), 8=I(sentAt)
  var data = s.getRange(editedRow, 1, 1, 9).getValues()[0]; 
  var userId = data[1];      // Column B
  var gptDraft = data[5];    // Column F
  var editedText = data[7];  // Column H (editedText)

  // Determine the text to send: Use editedText (Column H) if available, otherwise use gptDraft (Column F)
  ...
  if (!userId || !textToSend) {
    ...
    s.getRange(editedRow, 7).setValue('送信エラー:情報不足'); // Update status (Column G) to indicate error
    return;
  }
  ...
  if (pushLine(userId, textToSend)) {
    ...
    s.getRange(editedRow, 7).setValue('送信済');       // Column G (status)
...
/**
 * 状態列（G列）にドロップダウンリストを設定する関数
 * この関数を一度手動で実行すると、G列に選択式のドロップダウンリストが設定されます。
 */
function setupStatusColumnValidation() {
  ...
  // G列（状態列）の範囲を定義、2行目から開始（1行目はヘッダーと仮定）
  var statusRange = sheet.getRange(2, 7, lastRow - 1); // G列は7番目の列
  ...
  SpreadsheetApp.getUi().alert('状態列（G列）にドロップダウンリストを設定しました。');
}